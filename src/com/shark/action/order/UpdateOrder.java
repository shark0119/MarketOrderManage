...
public class UpdateOrder extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!CommonUtil.isEmpty(request.getParameter("operation"))
				&& request.getParameter("operation").equals("update")) {
			// 来源2 2.查看页面点击提交
			...
			int cp_id = -1;
			try {
				cp_id = OtherServiceUtil.getCP_id(Integer.parseInt(providerId), Integer.parseInt(productId));
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			if (!CommonUtil.getOrderService().updateOrder(order)) {
