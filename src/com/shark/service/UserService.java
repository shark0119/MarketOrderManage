...
public class UserService {
	 ...
	 * 
	 * @param user
	 *            包含用户有效信息的实体类对象，其ID和创建日期由数据库自动生成
	 * @return 成功返回插入的user对象 失败返回null
	 */
	public User addUser(User user) {
		String sql = "{?=call addUser (?, ?, ?, ?, ?, ?, ?)}";
		gs = new UserSql(sql, user.getName(), MD5Util.string2MD5(user.getPwd()), user.getSex(),
				new java.sql.Date(user.getBirth().getTime()), user.getMobile(), user.getAddress(), user.getRid());
	...
	public List<User> getUserList(Pager pager) {
		...
		String sql = "select * from " + " (select rownum rn, t1.* from (select * from mk_user) t1) "
				+ " where rn > ? and rn <= ? ";
	 ...
	 */
	/*
	 * public User loginVerify (String username, String pwd){ String sql =
	 * "select * from mk_user where username=? and password = ? "; gs = new
	 * UserSql (sql, username, pwd); return ud.getUser(gs) ; }
	 */
	...
	public List<User> getUserList(Pager pager, User condition) {
		...
		pageIndex = pager.getPageIndex();
		pageSize = pager.getPageSize();
		pager.setTotalCount(CommonUtil.getCount(new UserSql(
				" select count(1) from mk_user where 1=1" + "and username like '%" + condition.getName() + "%'")));
		String sql = "select * from " + " (select rownum rn, t1.* from (select * from mk_user where 1=1"
				+ "and username like '%" + condition.getName() + "%'" + " ) t1)  where rn > ? and rn <= ? ";
		// 条件字符串拼接
		List<User> users = ud.getUserList(new UserSql(sql, (pageIndex - 1) * pageSize, pageIndex * pageSize));
		for (User user : users) {
			user.setRname(CommonUtil.getRoleService().getRoleName(user.getRid()));
	...
	public boolean updateUser(User user) {
		if (!CommonUtil.isEmpty(user.getPwd())) {
			String sql = " update mk_user set username=?, password=?, sex=?, birth=?, mobile=?, address=?, roleid=? where id = ? ";
			...
			String sql = " update mk_user set username=?, sex=?, birth=?, mobile=?, address=?, roleid=? where id = ? ";
			if (-1 != ud.updateUser(new UserSql(sql, user.getName(), user.getSex(),
					new java.sql.Date(user.getBirth().getTime()), user.getMobile(), user.getAddress(), user.getRid(),
					user.getId())))
				return true;
			return false;			
